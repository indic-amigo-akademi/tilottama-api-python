def words_to_number(word_string: str) -> int | None:
    ...
    for word in words:
        if word in units:
            ...
            if current_hundreds_block == 0: # Handles cases like "thousand" (meaning 1 thousand)
                current_hundreds_block = 1
            
            # Add the current block's value multiplied by its large scale
            total_sum += current_hundreds_block * large_scale_multipliers[word]
            current_hundreds_block = 0 # Reset for the next block of hundreds/tens/units
        elif word == "and":
            continue # Ignore "and"
        elif word == "a": # Handle "a hundred" implicitly by allowing next word to be "hundred"
            continue # 'a' is context-dependent, handled when 'hundred' or another number follows.
        else:
            print(f"Warning: Unknown word '{word}' encountered. Conversion might be incomplete.")
